...
meets in a deployment. The client is held to the queue client contract; the
tests here cover what only HTTP adds: the ``get_task`` read-back, status codes,
connection failures and the manager's long-poll deadline.
"""
...
class TestHttpQueueClient(QueueClientContract):
    @pytest.fixture
    def queue_client(self, client: HttpQueueClient) -> QueueClient:
        return client
